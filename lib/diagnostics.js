...
export function buildDiagnosticsView(state, limits = {}) {
  ...
  return {
    ...
    catalogue: {
      status: state.catalogDiagnostics?.status ?? 'cold',      discovered: discovered.length,
     ...
     * came from — the bundled release-time file, or a live models.dev read — and
     * what could not be read. Forwarded verbatim (`official-runtime.js` owns the
     * shape) because it is already a flat, serializable summary and a second

 ...
 *
 * TWO readers share these rules, and that is the point of keeping them here:
 *
 *   - the BUILD-time refresh (`scripts/fetch-official-baseline.mjs`) writes the
 *     bundled file that ships with the plugin — the offline floor;
 *   - the RUNTIME layer (`official-runtime.js`) fetches the same files for a
 *     model the bundle does not know (or has not re-read recently), so a model
 *     the gateway started advertising today carries its declared numbers today
 *     instead of the conservative defaults until the next release.
 *
...
/**
 * The files a RUNTIME fetch has to read to resolve one model, derived from what
 * is already known — the OpenCode Go entry, and the record a previous run left
 * behind.
 *
 * The build-time refresh (`scripts/fetch-official-baseline.mjs`) can warm a
 * cache because it knows every id up front and can enumerate a local checkout.
 * A runtime fetch has neither: it is asked about ONE id, over a transport that
 * cannot list directories. So the plan is derived the same way
 * {@link resolveOfficialSources} would derive it, minus the parts that need a
 * listing:
 *
 *   - a previous record's `sources` win when there is one (the same rule the
 *     network refresh uses: a refresh costs exactly the files it reuses);
 *   - otherwise the direct rules apply: the OpenCode Go entry names a
 *     `base_model`, and that lab's provider directory plus the canonical
 *     `models/<lab>/<slug>.toml` are the candidates.
 *
 * An id whose OpenCode Go entry has no `base_model` cannot be resolved without
 * enumeration, and the plan says so (`lab: undefined`) instead of guessing a
 * lab from the id — a wrong guess would attach another vendor's numbers.
 *
 * @param {object} params - planning inputs.
 * @param {string} params.id - the OpenCode Go model id.
 * @param {string | undefined} params.ocgText - the text of `providers/opencode-go/models/<id>.toml`, when it exists.
 * @param {object | undefined} [params.previous] - a record from an earlier resolution, when there is one.
 * @returns {{ paths: string[], lab: string | undefined, slug: string | undefined, how: string }}
 *   the paths to read (deduplicated, ocg first) and what the plan was derived from.
 */
export function officialFetchPlan({ id, ocgText, previous }) {
  const ocgPath = `providers/opencode-go/models/${id}.toml`
  const recorded = previous?.sources === undefined
    ? []
    : Object.values(previous.sources).filter((path) => typeof path === 'string' && path.length > 0)
  if (recorded.length > 0) {
    return {
      paths: [...new Set([ocgPath, ...recorded])],
      lab: previous.lab,
      slug: previous.slug,
      how: 'recorded-sources',
    }
  }

  const ref = ocgText === undefined ? undefined : splitRef(parseCached(ocgPath, ocgText)?.base_model)
  if (ref === undefined) return { paths: [ocgPath], lab: undefined, slug: undefined, how: 'no-base-model' }

  const slug = ref.slug ?? id
  const paths = [ocgPath]
  for (const dir of providerDirsFor(ref.lab)) {
    paths.push(`providers/${dir}/models/${id}.toml`, `providers/${dir}/models/${slug}.toml`)
  }
  paths.push(`models/${ref.lab}/${slug}.toml`, `models/${ref.lab}/${id}.toml`)
  paths.push(`providers/${ref.lab}/provider.toml`)
  return { paths: [...new Set(paths)], lab: ref.lab, slug, how: 'base_model' }
}

...
export class OpenCodeGoAdapter extends LlmAdapter {
  ...
  async #snapshotNow() {
    ...
    if (this.snapshot !== undefined && this.snapshot.options === optionsNow && this.snapshot.idsKey === idsKey) {
    ...
    const provider = piAi.createProvider({
      ...
      auth: {
        apiKey: {
          ...
          // only reached for an id pi-ai looks up on its own.
          resolve: async () => ({ auth: { apiKey: await this.config.resolveApiKey() } }),
        },
      },
      api: apiImpls,
    })
    ...
    this.snapshot = { options: optionsNow, idsKey, piAi, models: collection, provider }
    return this.snapshot
  }
  ...
  async *stream(options) {
    const { piAi, models, options: connection } = await this.#snapshotNow()
    ...
    try {
      for await (const chunk of streamWithProtocolChain({
        chain,
        attempt,
      ...
      if (timeoutOf(error, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(`${PKG} stream idle timeout after ${connection.streamIdleTimeoutMs}ms`, 'TIMEOUT', { cause: error })
      }
      if (options.signal?.aborted === true) {
        throw new LlmError(`${PKG} request aborted by caller`, 'ABORTED', { cause: error })
      }
      throw new LlmError(`${PKG} stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
  }
